...
func main() {
	...
	model, err := gemini.NewModel(ctx, config.ModelName("correlator"), &genai.ClientConfig{
	...
	correlatorAgent, err := agents.NewCorrelatorRoot(model)
	if err != nil {
	...
	cfg := &launcher.Config{
		AgentLoader: agent.NewSingleLoader(correlatorAgent),
