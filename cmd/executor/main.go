...
func main() {
	...
	model, err := gemini.NewModel(ctx, config.ModelName("executor"), &genai.ClientConfig{
	...
	executorAgent, err := agents.NewExecutorRoot(model)
	if err != nil {
	...
	cfg := &launcher.Config{
		AgentLoader: agent.NewSingleLoader(executorAgent),
