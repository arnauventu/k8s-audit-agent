...
func main() {
	...
	model, err := gemini.NewModel(ctx, config.ModelName("verifier"), &genai.ClientConfig{
	...
	verifierAgent, err := agents.NewVerifierRoot(model)
	if err != nil {
	...
	cfg := &launcher.Config{
		AgentLoader: agent.NewSingleLoader(verifierAgent),
